...
class Table implements RepositoryInterface, EventListenerInterface, EventDispatcherInterface, ValidatorAwareInterface
{
	...
	public function getChanges($entity, $model, $accountId)
	{
		$encodeEntity = json_encode($entity, true);
		$encodeEntity = json_decode($encodeEntity);

		if (!is_object($entity) || !is_object($encodeEntity))
			return;
		...
		if ($entity->isNew())
		{
			$this->changes[] =
				[
					...
					"new" => json_encode($entity),
					"account_id" => $accountId
				];
		}

		foreach ($encodeEntity as $key => $value)
		{
			$original = $entity->getOriginal($key);

			if (is_object($original) &&
				get_class($original) == "Cake\I18n\FrozenTime")
				$original = $original->toAtomString();
			...
			if (is_array($value) ||
				...
				is_object($original))
			{
				$this->getChanges($entity[$key], $key, $accountId);
			}
	...
	public function afterSave($event, $entity, $options)
	{
		...
		if (isset($_SESSION["Auth"]["User"]["id"]))
			$accountId = $_SESSION["Auth"]["User"]["id"];
		else
			$accountId = null;

		$this->belongsTo("DbChangeLog");

		$this->changes = [];

		$this->getChanges($entity, $model, $accountId);

		$this->DbChangeLog->saveChanges($this->changes);
	}
}
